...
import org.siit.homework.week11.bookingApp.RoomPrice;
...
public class RelationTableService {
    ...
    public static List<RoomPrice> selectRoomWithPrice(Connection connection) {
        List<RoomPrice> roomWithPriceList = new ArrayList<>();
        try (PreparedStatement preparedStatement = connection.prepareStatement(RELATION_TABLE_SELECT);
             ResultSet resultSet = preparedStatement.executeQuery()) {
            while (resultSet.next()) {
                RoomPrice room = RoomPrice.builder()
